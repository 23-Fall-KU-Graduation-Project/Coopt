...
def make_target_index_vector(y, n):
    result = F.one_hot(y, num_classes=10)

    for index, data in enumerate(result):
        indices = list(range(10))
        indices.remove(y[index])
        random.shuffle(indices)
        for j in indices[:n]:
            data[j] = -1
            
    return result.type(torch.FloatTensor).to(device)

class LinfPGDAttackTargeted(object):
    def __init__(self, model):
        self.model = model

    def perturb(self, x_natural, y, n):
        x = x_natural.detach()
        ...
        for i in range(k):
            ...
            with torch.enable_grad():
                logits = self.model(x)
                y_targeted = make_target_index_vector(y, n)
                loss = F.cross_entropy(logits, y_targeted) / (n + 1)
...
parser.add_argument("--target_num",default=1, type=int, help="The number of target labels.")
parser.add_argument("--device",default=0, type=int, help="The number of gpu device.")
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        adv, y_targeted = adversary_targeted.perturb(inputs, targets, args.target_num)
        adv_outputs = net(adv)
        loss = criterion(adv_outputs, y_targeted)
        loss.backward()
...
def test(epoch):
    ...
    if 100. * benign_correct / total > best_clean_val:
        torch.save(state, './checkpoint/pgd_at/' + file_name + "_clean_best")
        print('Clean Best Model Saved!')
        best_clean_val = 100. * benign_correct / total

    if 100. * adv_correct / total > best_adv_val:
        torch.save(state, './checkpoint/pgd_at/' + file_name + "_adv_best")
    ...
    if not os.path.isdir('checkpoint/pgd_at'):
        os.mkdir('checkpoint/pgd_at')
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir = f"logs/pgd_at/{time}") # directory for tensorboard logs
    log_dir = "./pgd_at/best_checkpoint" # directory for model checkpoints

    net = net.to(device)
    # net = torch.nn.DataParallel(net)
    # cudnn.benchmark = True

    adversary_targeted= LinfPGDAttackTargeted(net)
